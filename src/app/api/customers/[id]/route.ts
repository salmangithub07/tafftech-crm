...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  const existing = await query("SELECT id FROM customers WHERE id = ? AND tenant_id = ?", [id, tenantId]);
  if (!existing.length) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const d = parsed.data;
  await execute(
    `UPDATE customers SET name=?, product=?, email=?, phone=?, address=?, notes=?, status=?, visited=? WHERE id=? AND tenant_id=?`,
    [d.name, d.product, d.email, d.phone, d.address, d.notes, d.status, d.visited ? 1 : 0, id, tenantId]
  );
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  await execute("DELETE FROM customers WHERE id = ? AND tenant_id = ?", [id, tenantId]);
