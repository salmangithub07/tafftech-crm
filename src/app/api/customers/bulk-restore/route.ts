...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = bulkRestoreSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const res = await execute(
      `UPDATE customers SET is_trashed = 0 WHERE id IN (${placeholders}) AND tenant_id = ?`,
      [...ids, tenantId]
    );
    ...
    logActivity({
      ...
      action: "↺ Restored Customers from Trash",
      entityType: "customer",
      entityLabel: `${ids.length} customer(s) restored from trash`,
    });

    return NextResponse.json({ ok: true, count: res.affectedRows });
  } catch (err: any) {
    console.error("Error restoring customers:", err);
    return NextResponse.json({ error: err.message || "Failed to restore customers" }, { status: 500 });
  }
}
