...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await execute(
      `UPDATE appointments SET is_trashed = 0 WHERE id IN (${placeholders}) AND tenant_id = ?`,
      [...ids, tenantId]
    );
    ...
    logActivity({
      ...
      action: "↺ Restored Appointments from Trash",
      entityType: "appointment",
      entityLabel: `${ids.length} appointment(s) restored from trash`,
    });
