...
export function BillForm() {
  ...
  const {
    ...
    defaultValues: {
      ...
      items: [
        {
          product_id: prefillProductId ? Number(prefillProductId) : null,
          product_name: "",
          hsn_code: "",
  ...
  const grandTotal = Math.max(0, subtotal + Number(watchedTaxAmount) - Number(watchedDiscount));
  const remainingDue = Math.max(0, grandTotal - Number(watchedPaidAmount));
  ...
  // Adjust paid amount when payment status changes
  React.useEffect(() => {
  ...
  // Load initial options and prefilled data
  React.useEffect(() => {
    ...
    if (prefillQuotationId) {
      promises.push(fetch(`/api/quotations/${prefillQuotationId}`).then((r) => (r.ok ? r.json() : null)));
    }

    Promise.all(promises)
      .then(([cRes, pRes, bsRes, sRes, qData]) => {
        const custData: Customer[] = cRes.data || [];
        const prodData: Product[] = pRes.data || [];
        ...
        if (qData) {
          ...
          const foundProd = prodData.find((p) => p.id === Number(prefillProductId));
          if (foundProd) {
            setValue("items.0.product_id", foundProd.id);
            setValue("items.0.product_name", foundProd.name);
            setValue("items.0.hsn_code", foundProd.hsn_code || foundProd.sku || "");
            setValue("items.0.unit_price", Number(foundProd.price || 0));
            setValue("items.0.quantity", prefillQuantity ? Number(prefillQuantity) : 1);
          }
  ...
  }, [prefillQuotationId, prefillCustomerId, prefillProductId, prefillQuantity, reset, setValue]);
  ...
  async function onSubmit(values: FormValues) {
    try {
      const res = await fetch("/api/bills", {
        ...
        body: JSON.stringify({
          ...values,
          quotation_id: prefillQuotationId ? Number(prefillQuotationId) : null,
        }),
      });
      ...
      const targetId = savedData.id || savedData.bill?.id;
      if (targetId) {
  ...
  if (loadingOptions) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-sm font-medium text-muted-foreground">Preparing invoice form...</p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-16 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Create Tax Invoice / Bill</h1>
            <p className="text-xs text-muted-foreground">Generate an official GST invoice and record sales</p>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="mt-1">
                  <DatePicker
                    ...
                    onChange={(val) => setValue("bill_date", val)}
                  />
                </div>
              </div>
              <div>
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="credit">Credit / Cheque</SelectItem>
